...
func newClient(newFileNumber int64, fullname string, dni int, phoneNumber int, address string) Client {
	return Client{
...
func validateFieldsOfClient(fileNumber int64, fullname string, dni, phoneNumber int, address string) (bool, error) {
	...
	if phoneNumber == 0 {
		return false, &ClientError{
			Status:  500,
			Message: "El teléfono no puede ser 0",
		}
	}
	...
	if address == "" {
		return false, &ClientError{
			Status:  500,
			Message: "El domicilio nes requerido",
		}
	}
...
func main() {
	...
	if newFileNumber < 0 {
		panic("El número de legajo no puede estar vacio")
	}

	if !clientExists(newFileNumber, clients) {
		fullname := "Diego Parra"
		dni := 1234567
		phoneNumber := 55555
		address := "cr 88f ·57"
		...
		if validated {
			client := newClient(newFileNumber, fullname, dni, phoneNumber, address)
