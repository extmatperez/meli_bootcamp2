...
func TestGetOneServiceSQL(t *testing.T) {
	...
	assert.Equal(t, productoNuevo.Nombre, personaCargada.Nombre)
	assert.Equal(t, productoNuevo.Color, personaCargada.Color)
	// assert.Nil(t, misPersonas)
}
...
func TestUpdateServiceSQL(t *testing.T) {
	...
	personaCargada, _ := service.Update(productoNuevo)
...
func TestGetByNameServiceSQL(t *testing.T) {
	...
	personaCargada := service.GetByName("producto test")
