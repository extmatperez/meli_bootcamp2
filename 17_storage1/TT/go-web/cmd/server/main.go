...
func main() {
	...
	repositorySQL := products.NewRepositorySQL(db.StorageDB)
	...
	productsRoute.GET("", controllerSQL.GetAll())
	productsRoute.GET("/:id", controllerSQL.FindById())
	productsRoute.POST("", controllerSQL.Store())
	productsRoute.PUT("/:id", controllerSQL.Update())
	productsRoute.DELETE("/:id", controllerSQL.Delete())
	productsRoute.PATCH("/:id", controllerSQL.UpdateNameAndPrice())
