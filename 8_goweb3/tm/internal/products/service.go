...
func (s *service) Update(id int, name, color string, price float64, stock, code int, isPublished bool, createdAt string) (Product, error) {
	prod, err := s.repository.Update(id, name, color, price, stock, code, isPublished, createdAt)
	if err != nil {
		return Product{}, err
	}
	return prod, nil
